...
class Process:
    ...
    def is_bad_file(self, run):
        ...
        key = 'master_deck'
        if key not in run or set(run[key]).issubset(Process.BASE_GAME_CARDS_AND_UPGRADES) is False:
            # deck = data[key]
            # logger.info(f'Modded file. Cards: {deck - BASE_GAME_CARDS_AND_UPGRADES}')
            return True

        key = 'relics'
        if key not in run or set(run[key]).issubset(Process.BASE_GAME_RELICS) is False:
            return True
